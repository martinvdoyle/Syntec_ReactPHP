--------------------------------------------------------
--  DDL for Function GENERATE_MEGA_MENU_DESKTOP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "WEB"."GENERATE_MEGA_MENU_DESKTOP" (
  p_app_id           IN number,
  p_app_alias        IN VARCHAR2,
  p_workspace_url    IN VARCHAR2,
  p_business         IN varchar2,
  p_website          IN varchar2,
  p_main_menu        IN varchar,
  p_main_menu_id     IN varchar,
  p_sub_menu_l1_id   IN varchar,
  p_sub_menu_l2_id   IN varchar,
  p_sub_menu_l3_id   IN varchar
) RETURN CLOB IS
  v_menu_html CLOB := '';
...
BEGIN
  ...
  <i class="fa fa-envelope"></i>info@syntec.ie
            ...
            <ul class="mobile-hide">
                ...
                <li><a class="shape" href="javascript:void(0)" id="openSiteMap"><i class="fa fa-sitemap"></i> Site map</a></li>
            </ul>
...
<header class="top-head header-9 sticky-nav" data-sticky="true">
  <div class="nav-container-syn">
    ...
    <nav class="menu-wrapper-syn">
      <div class="container-syn-mainmenu">
        ...
        <div id="logo-mainmenu" class="logo fx animated fadeInLeft" data-animate="fadeInLeft">
            <a href="'||
              CASE p_website
                WHEN 'Syntec Group' THEN  syntec_page_url(1, 1) ||'" onclick="updateMenuItems(''Syntec Group'', ''1000'', '''','''','''','''','''','''','''',''Syntec Group'', ''main_menu'', '''', ''L0'')"'
                ||'><img alt="'||p_website||'" src="'||p_workspace_url || 'assets/images/Syntec_Group_Logo_Menu.png" ></a>'
                WHEN 'Syntec Scientific' THEN  syntec_page_url(10, 1) ||'" onclick="updateMenuItems(''Syntec Scientific'', ''4000'', '''','''','''','''','''','''','''',''Syntec Scientific'', ''main_menu'', ''Ireland'', ''L0'')"'
                ||'><img alt="'||p_website||'" src="'||p_workspace_url || 'assets/images/Syntec_Scientific_Logo_Menu.png" ></a>'
                WHEN 'Syntec International' THEN  syntec_page_url(20, 1) ||'" onclick="updateMenuItems(''Syntec International'', ''5000'', '''','''','''','''','''','''','''',''Syntec International'', ''main_menu'', ''International'', ''L0'')"'
                ||'><img alt="'||p_website||'" src="'||p_workspace_url || 'assets/images/Syntec_International_Logo_Menu.png"   ></a>'
                WHEN 'SyS Laboratories' THEN  syntec_page_url(6, 1) ||'" onclick="updateMenuItems(''SyS Laboratories'', ''6000'', '''','''','''','''','''','''','''',''SyS Laboratories'', ''main_menu'', ''Ireland'', ''L0'')"'
                ||'><img alt="'||p_website||'" src="'||p_workspace_url || 'assets/images/Syntec_SysLabs_Logo_Menu.png" ></a>'
                ELSE syntec_page_url(1, 1) ||'" onclick="updateMenuItems(''Syntec Group'', ''1000'', '''','''','''','''','''','''','''',''Syntec Group'', ''main_menu'', '''', ''L0'')"'
                ||'><img alt="'||p_website||'" src="'||p_workspace_url || 'assets/images/Syntec_Group_Logo_Menu.png" ></a>'
              END
            ||'
        </div>
        ...
        <div id="main_menu" class="main-menu-container-mainmenu">
          <nav class=" top-nav nav-animate to-bottom ">'||
              /* IMPORTANT: desktop UL builder only */
              GENERATE_MENU_NAV_UL_LI_DESKTOP(
                p_app_id, p_app_alias, p_workspace_url, v_business, p_website,
                p_main_menu, p_main_menu_id, p_sub_menu_l1_id, p_sub_menu_l2_id, p_sub_menu_l3_id
              )
          ||'</nav>
        </div>
      ...
      <!-- Business / Language selectors (unchanged) -->
    ...
    </nav><!-- /menu-wrapper-syn -->
  </div><!-- /nav-container-syn -->
</header>
';
