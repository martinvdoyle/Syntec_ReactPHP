--------------------------------------------------------
--  DDL for Function GENERATE_MENU_NAV_UL_LI_DESKTOP_29APR2026
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "WEB"."GENERATE_MENU_NAV_UL_LI_DESKTOP_29APR2026" (
...
RETURN CLOB IS
    ...
    -- Phase 1 helper: unified Page 22 hierarchy-mode target URL
    function build_page22_href(
      p_diagnostic_id   in varchar2,
      p_product_line_id in varchar2,
      p_context_source  in varchar2
    ) return varchar2
    is
    begin
      return 'f?p=' || p_app_id || ':22:' || v('APP_SESSION')
        || '::::'
        || 'P0_NAV_MODE,P0_DIAGNOSTIC_ID_CTX,P0_PRODUCT_LINE_ID_CTX,P0_CONTEXT_SOURCE'
        || ':HIERARCHY,'
        || nvl(p_diagnostic_id,'')
        || ','
        || nvl(p_product_line_id,'')
        || ','
        || nvl(p_context_source,'');
    end build_page22_href;
...
BEGIN
        ...
        DECLARE v_sub_menu_exists NUMBER; BEGIN
          ...
          IF v_sub_menu_exists > 0 THEN
            ...
            v_menu_html := v_menu_html || '<ul id="' || v_menu_name || '" '||
               (CASE 
                    WHEN v_menu_class LIKE '%mega-menu%' and v_device_type = 'DESKTOP' THEN
                      CASE
                        WHEN v_has_col_groups > 0 THEN 'class="mega-menu_pos mega-menu_pos-grouped" style="display:flex !important;flex-flow:column wrap !important;align-content:stretch !important;height:560px !important;overflow:hidden !important;"'
                        ELSE 'class="mega-menu_pos"'
                      END
                    ELSE NULL 
                END) || '> ';
            ...
            FOR r_sub IN (
              ...
              IF INSTR(v_menu_class, 'col-md-4') > 0 OR INSTR(v_menu_class, 'col-md-3') > 0 THEN
                ...
                IF v_sub_sub_menu_exists > 0 THEN
                  ...
                  FOR r_sub_sub IN (
                    ...
                    v_menu_href := case
                                     ...
                                     then build_page22_href(
                                            r_sub_sub.discipline_id,
                                            r_sub_sub.product_group_id,
                                            case
    ...
    -- Deterministic grouped mega-menu layout: rebuild grouped UL into 4 explicit columns
    v_menu_html := v_menu_html || '
<script>
(function () {
  function applyGroupedMegaColumns() {
    var grouped = document.querySelectorAll(''ul.mega-menu_pos.mega-menu_pos-grouped'');
    grouped.forEach(function (ul) {
      if (!ul || ul.getAttribute(''data-grouped-applied'') === ''Y'') return;
      ...
      items.forEach(function (li) {
        ...
        var m = cls.match(/mega-col-group-([1-4])/);
        var g = m ? parseInt(m[1], 10) : 1;
        cols[g].push(li);
      });

      ul.innerHTML = '''';
      ul.setAttribute(''style'', ''display:flex !important;flex-flow:row nowrap !important;align-items:stretch !important;box-sizing:border-box !important;height:auto !important;overflow:visible !important;padding:0 !important;margin:0 !important;position:absolute !important;transform:none !important;right:auto !important;z-index:9999 !important;'');

      [1,2,3,4].forEach(function (g, idx) {
        var col = document.createElement(''li'');
        col.className = ''mega-col-wrap col-md-3'';
        col.setAttribute(''style'', ''display:block !important;flex:0 0 25% !important;max-width:25% !important;width:25% !important;float:none !important;box-sizing:border-box !important;border-left:'' + (idx === 0 ? ''0'' : ''1px solid #e9e9e9'') + '';padding:12px 14px 6px;vertical-align:top;'');

        var inner = document.createElement(''div'');
        inner.className = ''mega-col-inner'';
        inner.setAttribute(''style'', ''display:block !important;'');

        cols[g].forEach(function (node, idxInCol) {
          var blockGap = (idxInCol > 0) ? ''margin-top:22px !important;padding-top:10px !important;'' : ''margin-top:0 !important;padding-top:0 !important;'';
          node.setAttribute(''style'', ''display:block !important;width:100% !important;float:none !important;'' + blockGap + ''margin-bottom:0 !important;padding-left:0 !important;padding-right:0 !important;break-inside:auto !important;-webkit-column-break-inside:auto !important;page-break-inside:auto !important;'');
      ...
      ul.setAttribute(''data-grouped-applied'', ''Y'');

      /* Exact anchor: start under #logo-mainmenu, end under #Business (Ireland selector area),
         and position directly under the nav row (not viewport-fixed). */
      var nav = document.querySelector(''.menu-wrapper-syn'') || document.querySelector(''.nav-container-syn'');
      var logo = document.querySelector(''#logo-mainmenu'');
      var biz  = document.querySelector(''#Business'');
      var triggerLi = ul.parentElement;
      if (nav && logo && biz && triggerLi) {
        nav.style.setProperty(''position'', ''relative'', ''important'');
        var n = nav.getBoundingClientRect();
        var l = logo.getBoundingClientRect();
        var b = biz.getBoundingClientRect();
        var t = triggerLi.getBoundingClientRect();

        var leftAbs = l.left;
        var rightAbs = b.right;
        var widthAbs = Math.max(0, rightAbs - leftAbs);
        var leftRel = Math.round(leftAbs - n.left);
        var topRel = Math.round(t.bottom - n.top);
        var widthRel = Math.round(widthAbs);

        ul.style.setProperty(''left'', leftRel + ''px'', ''important'');
        ul.style.setProperty(''top'', topRel + ''px'', ''important'');
        ul.style.setProperty(''width'', widthRel + ''px'', ''important'');
        ul.style.setProperty(''min-width'', widthRel + ''px'', ''important'');
        ul.style.setProperty(''max-width'', widthRel + ''px'', ''important'');
  ...
  if (document.readyState === ''loading'') {
    document.addEventListener(''DOMContentLoaded'', applyGroupedMegaColumns);
  } else {
    applyGroupedMegaColumns();
  }

  window.addEventListener(''load'', applyGroupedMegaColumns);
})();
</script>';
